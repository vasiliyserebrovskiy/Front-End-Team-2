			...
			<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 lg:grid-cols-5'>
				{products.map(p => (
					<div
						...
						className=' bg-white rounded-3xl  flex flex-col items-center text-center transition transform hover:scale-130 hover:shadow-2xl
...
before:opacity-0 group-hover:before:opacity-100 before:transition-opacity before:duration-300 before:rounded-t-3xl rounded-tl-[220px] rounded-tr-[220px] rounded-b-[40px] max-w-[230px] w-[90%] sm:w-[290px] md:w-[290px] pt-16 pb-3 gap-x-0 mb-8 hover:-translate-y-10 shadow-xl'
					>
						...
						<Link to={`/card/${p.itemID}`} className='bg-pink-500 hover:bg-pink-600 hover:scale-108 text-white px-6 py-2 mb-2 rounded-full font-semibold transition transform hover:shadow-2xs'>
							Buy now
						</Link>
					</div>
				))}
